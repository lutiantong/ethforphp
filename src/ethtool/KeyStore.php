...
class KeyStore {
    ...
    static function save($privateKey, $password, $dir, $file_salt) {
        ...
        $path = $dir;
        $hashdir = $hashdir2 = sha1($password); //hash目录
        $hashdir = '/'.substr($hashdir, 2, 2) . '/' . substr($hashdir, 6, 2) . '/';
        $path .= $hashdir;
        if (!is_dir($path)) {
        ...
        $wallet = $path . md5(substr($address, 2) . $file_salt) . '.json';
